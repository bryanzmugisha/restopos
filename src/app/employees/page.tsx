...
const blank = () => ({
  name: '', email: '', pin: '', role: 'WAITER', position: '',
  salaryType: 'MONTHLY', salaryAmount: 0,
  joiningDate: new Date().toISOString().slice(0, 10), password: '',
})
    ...
    try {
      if (modal === 'add') {
        ...
        if (res.ok) {
          ...
          setModal(null); showToast('✅ Employee added')
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: C.bg, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', padding: '8px 12px', borderBottom: `1px solid ${C.b}`, flexShrink: 0, overflowX: 'auto' }}>
        {[
          ...
          <div key={k.label} style={{ background: C.s, border: `1px solid ${C.b}`, borderRadius: '8px', padding: '8px 12px', minWidth: '100px', flexShrink: 0 }}>
            <p style={{ fontSize: '18px', fontWeight: '800', color: k.color, margin: 0 }}>{k.value}</p>
      ...
      <div style={{ padding: '8px 12px', borderBottom: `1px solid ${C.b}`, flexShrink: 0 }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search staff..." style={inputStyle} />
      </div>
            ...
            <div key={emp.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderBottom: `1px solid ${C.b}`, opacity: emp.isActive ? 1 : 0.5 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '10px', padding: '1px 6px', borderRadius: '4px', background: (roleColors[emp.user.role] ?? C.br) + '22', color: roleColors[emp.user.role] ?? C.br, fontWeight: '700' }}>{emp.user.role.replace(/_/g, ' ')}</span>
              ...
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                <button onClick={() => clockToggle(emp)} style={{ padding: '6px 10px', borderRadius: '7px', border: `1px solid ${cs?.in ? '#7f1d1d' : '#14532d'}`, background: 'transparent', color: cs?.in ? '#ef4444' : '#22c55e', cursor: 'pointer', fontSize: '11px', fontWeight: '600' }}>
                  {cs?.in ? 'Out' : 'In'}
                </button>
                {isAdmin && <>
                  <button onClick={() => openEdit(emp)} style={{ padding: '6px 10px', borderRadius: '7px', border: `1px solid ${C.b}`, background: 'transparent', color: C.m, cursor: 'pointer', fontSize: '11px' }}>✏️</button>
                  <button onClick={() => openDelete(emp)} style={{ padding: '6px 10px', borderRadius: '7px', border: '1px solid #7f1d1d', background: 'transparent', color: '#ef4444', cursor: 'pointer', fontSize: '11px' }}>🗑️</button>
                </>}
              </div>
            </div>
          )
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 50, display: 'flex', alignItems: 'flex-end' }} onClick={() => setModal(null)}>
          <div style={{ background: '#1c1c1e', borderTop: `1px solid ${C.b}`, borderRadius: '20px 20px 0 0', width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: '20px 16px', paddingBottom: 'max(20px,env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <input value={form.name} onChange={e => setForm(x => ({ ...x, name: e.target.value }))} placeholder="e.g. James Okello" style={inputStyle} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label style={labelStyle}>PIN *</label>
                <input value={form.pin} onChange={e => setForm(x => ({ ...x, pin: e.target.value.replace(/\D/g, '').slice(0, 6) }))} placeholder="4-6 digits" inputMode="numeric" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Email</label>
                <input type="email" value={form.email} onChange={e => setForm(x => ({ ...x, email: e.target.value }))} placeholder="optional" style={inputStyle} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <input value={form.position} onChange={e => setForm(x => ({ ...x, position: e.target.value }))} placeholder="e.g. Head Waiter" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Role *</label>
                <select value={form.role} onChange={e => setForm(x => ({ ...x, role: e.target.value }))} style={inputStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <select value={form.salaryType} onChange={e => setForm(x => ({ ...x, salaryType: e.target.value }))} style={inputStyle}>
                  <option value="MONTHLY">Monthly</option>
                  <option value="HOURLY">Hourly</option>
                </select>
              </div>
              <div>
                <label style={labelStyle}>Salary (UGX)</label>
                <input type="number" value={form.salaryAmount || ''} onChange={e => setForm(x => ({ ...x, salaryAmount: Number(e.target.value) }))} placeholder="0" style={inputStyle} />
              </div>
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={labelStyle}>Joining Date</label>
              <input type="date" value={form.joiningDate} onChange={e => setForm(x => ({ ...x, joiningDate: e.target.value }))} style={inputStyle} />
            </div>

            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setModal(null)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: `1px solid ${C.b}`, background: 'transparent', color: C.m, cursor: 'pointer', fontWeight: '600' }}>Cancel</button>
              <button onClick={save} disabled={saving} style={{ flex: 2, padding: '12px', borderRadius: '10px', border: 'none', background: C.br, color: 'white', cursor: 'pointer', fontWeight: '700', fontSize: '14px', opacity: saving ? 0.7 : 1 }}>
